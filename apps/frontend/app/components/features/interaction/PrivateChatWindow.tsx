...
const PrivateChatWindow = ({
  ...
  }, [fetchMessages, isOpen, order.id, order.productId]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Không thể gửi tin nhắn", { description: errorMessage });
    } finally {
      setIsSending(false);
    }
  };
  ...
  return (
    <Card className="border-primary/20 animate-in slide-in-from-bottom-10 fixed right-6 bottom-6 z-50 flex h-[500px] w-80 flex-col overflow-hidden shadow-xl duration-300 sm:w-96">
      <CardHeader className="bg-primary/5 flex flex-row items-center justify-between border-b p-3">
